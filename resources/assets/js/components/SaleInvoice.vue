<template>
  <div style="background-color: #efefef">
    ...
    <div class="no-user-address" v-if="!latestAddress">
      <router-link to="/wechat/address_edit?from=sale_invoice" class="add-address">添加地址</router-link>
    </div>
    <div class="user-address" v-else>
      ...
      <div class="user-address-update">
        <router-link
          to="/wechat/address_list?from=sale_invoice"
    ...
    <div class="sale-books">
      ...
      <div class="sale-book">
        <div class="book-name">
          运费(不参与优惠)
          <router-link to="/wechat/express_fee">
...
export default {
    ...
    this.$store.dispatch("user/latestAddress").then(res => {
      ...
      if (!res.data) {
        this.$dialog
          ...
          .then(() => {
            // confirm
            this.$router.push("/wechat/address_edit?from=sale_invoice");
          });
      } else {
        this.getWxConfig();
      }
  ...
  beforeRouteLeave: function(to, from, next) {
    ...
    if (
      to.path === "/wechat/address_edit" ||
      to.path === "/wechat/address_list" ||
      to.path === "/wechat/express_fee" ||
      to.path.indexOf("/wechat/sale_order/") == 0
    ) {
      next();
    } else {
      this.$dialog.confirm({
      ...
      if (this.order) {
        this.paySaleOrderWithWallet(this.order.id).then(res => {
          if (res.data.code && res.data.code == 500) {
            ...
            this.order = "";
            this.$router.replace("/wechat/sale_order/" + no);
          }
        });
      } else {
        this.createSaleOrder({
    ...
    wxPay: function(conf) {
      ...
      wx.chooseWXPay({
        ...
        success: function() {
          _this.paying = false;
          setTimeout(function() {
            _this.$router.replace("/wechat/sale_order/" + _this.order.no);
          }, 500);
        },
        fail: function() {
          _this.paying = false;
      ...
      axios
        ...
        .then(res => {
          if (_.isEmpty(res.data.paid_at)) {
            ...
            console.log("支付成功");
            this.createSuccess();
            this.$router.replace("/wechat/sale_order/" + this.order.no);
          }
        });
    },
