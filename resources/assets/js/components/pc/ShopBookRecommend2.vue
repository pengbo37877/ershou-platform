  ...
  <div class="shopOut">
    <div class="shop-book" ref="book">
      <van-card :price="skuPrice" :origin-price="skuOriginalPrice">
        ...
        <div slot="num" v-if="this.skus.length===0">
          ...
          <van-button
            round
            size="small"
            style="background-color: #3D404A;color:white;"
          ...
          <van-button
            round
            size="small"
            style="background-color: #606572;color:white"
            loading
            v-if="!inReminder && reminderAdding"
          >到货提醒</van-button>
          <van-icon
        ...
        <!--<van-icon name="cross" style="margin-left: 10px;" color="#aaaaaa" @click="showBanDialog"/>-->
        <!--</div>-->
        <div slot="num" v-else>
          <span class="discount">{{((skuPrice/skuOriginalPrice)*10).toFixed(1)}}折</span>
          <van-icon
      ...
      <!--<router-link :to="`/wechat/user/${skus[0].user.mp_open_id}`" v-if="skus[0].user">-->
      ...
      >
        ...
        <template slot="sku-header-price" slot-scope="props">
          <div class="van-sku__goods-price">
            ...
            <span class="van-sku__price-num">{{ props.price }}111111</span>
          </div>
        </template>
        <!-- 自定义 sku actions -->
      ...
      <van-dialog
        ...
        show-cancel-button
        @confirm="userBanBook"
        confirmButtonText="不再推荐"
      >
        <div style="text-align: center;padding: 20px 5px;">《{{book.name}}》</div>
      </van-dialog>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    userBanBook: function() {
      this.banBook(this.book);
    },
    ...mapActions("cart", [
      "addSkuToCart",
      "addBookToReminder",
