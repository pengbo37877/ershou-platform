<template>
  <div>
    ...
    <div class="search-suggestion" v-if="books.length===0 && !loading">
      <div class="s-title">你可以试试搜</div>
      <div class="s-content">
        <div class="s-tag" v-for="tag in tags" @click="tagClick(tag)">{{tag}}</div>
      </div>
    </div>
    <div class="search-suggestion" v-if="books.length===0 && !loading && historyList.length>0 &&tags.length>0" style="marginTop:20px">
...
export default {
  ...
  created: function() {
    this.$store.dispatch("search2/getRecommendTags");
  },
  mounted: function() {
    this.screenWidth =
      window.innerWidth ||
    ...
    search: function() {
      ...
      if (this.str.trim().length > 0) {
        if (window.localStorage.getItem("historyList")) {
          let oldHistory = JSON.parse(window.localStorage.getItem("historyList"));
          if (oldHistory.indexOf(this.str.trim()) == -1) {
            oldHistory.push(this.str.trim());
