<template>
  <div style="background-color: #efefef">
    <div class="no-user-address" v-if="!latestAddress">
      <router-link to="/pc/address_edit?from=recover_invoice" class="add-address">添加地址</router-link>
    </div>
    <div class="user-address" v-else>
      ...
      <div class="user-address-update">
        <router-link
          to="/pc/address_list?from=recover_invoice"
    ...
    <div class="recover-books" style="padding-bottom:64px;">
      ...
      <router-link to="/pc/review_standard" class="recover-review">
        实际收入以回流鱼收书审核后为准&nbsp;
        <van-icon name="arrow" />
      </router-link>
    </div>
        ...
        if (this.canRecover()) {
          this.createRecoverOrder({
            ...
            if (res.data.code && res.data.code === 500) {
              ...
              this.$router.replace("/pc/recover_order/" + res.data.no);
            }
          });
        } else {
          this.$dialog.alert({
