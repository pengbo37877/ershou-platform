    ...
    <van-tabs v-model="active" @change="onChange" sticky swipeable>
        <van-tab title="全部">
            ...
            <div class="cart-empty" v-if="items.length===0 && !loading">
                <img src="/images/empty.png" :style="imgStyle" style="opacity: .6" alt=""><br>
                去商店添加喜欢的书籍吧<br>
            </div>
            <cart-selling-item :item="item" :screen-width="screenWidth" v-for="item in sellingItems" :key="item.id" v-if="sellingItems.length>0"></cart-selling-item>
            <div class="no-sku-title" v-if="soldItems.length>0">以下暂时无货</div>
            <cart-sold-item :item="item" :screen-width="screenWidth" v-for="item in soldItems" :key="item.id"></cart-sold-item>
            <div class="no-sku-title" v-if="recommends.length>0">推荐购买</div>
            <cart-recommend-book :book="book" :screen-width="screenWidth" v-for="book in recommends" :key="book.id"></cart-recommend-book>

            <div style="width:100%;height:78px"></div>

            <div class="cart-bottom-bar" :style="{width: screenWidth+'px'}">
                <div class="cart-select-circle" @click="chooseAllSellingItems" v-show="sellingItems.length>0">
                ...
                <div class="cart-selected-price" style="color: #aaaaaa;text-align: center;" v-else>{{selectedPrice}}</div>
                <div class="cart-selected-desc" v-show="sellingItems.length>0">
                    <div class="cart-selected-express-fee" v-if="selectedPrice<99">快递费<span style="color: #ff4848">￥5元</span></div>
                    <div class="cart-selected-express-fee" v-if="selectedPrice<99">再买<span style="color: #ff4848">￥{{ Number(99-selectedPrice).toFixed(2) }}</span>包邮</div>
                    <div class="cart-selected-express-fee" v-if="selectedPrice>=99"><span style="color: #227E2C">已包邮</span></div>
                </div>
                <router-link to="/pc/sale_invoice" class="cart-selected-pay-btn" v-if="selectedItems.length>0">结算({{selectedItems.length}})</router-link>
        ...
        <van-tab title="到货提醒">
            ...
            <cart-reminder :reminder="reminder" :screen-width="screenWidth" v-for="reminder in reminders" :key="reminder.id" v-if="reminders.length>0"></cart-reminder>
            ...
            if (isbn) {
                axios.get('/wx-api/get_book_sale_sku?isbn='+isbn).then(res => {
                    if (res.data.code && res.data.code===500) {
                        ...
                        this.$router.replace('/pc/book/'+isbn+'?from=notify');
                    }
                });
            }
