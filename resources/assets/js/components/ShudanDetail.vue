<template>
  <div style="position: relative;" v-if="shudan">
    ...
    <router-link tag="div" to="/wechat/shop" class="back-home" v-if="isNotify">回首页</router-link>
    <router-link
      tag="div"
      to="/wechat/cart"
      class="back-cart"
  ...
  created() {
    ...
    this.$store.dispatch("user/getUser").then(res => {
      ...
      axios.get("/wx-api/shudan_users").then(res => {
        ...
        if (this.shudan) {
          let desc = this.shudan.desc
            .replace(/<\/?[^>]*>/g, "")
            .replace(/(\n)/g, "");
          let options = {
            title:
              this.users.nickname + "向你推荐了一个书单，这⾥是Ta的灵魂栖息地",
            desc: this.shudan.title + "-" + desc,
            link: window.location.href,
            imgUrl:
              window.localStorage.getItem("url") + "/images/image/logo.jpeg"
          };
          console.log(options);
          wxApi.wxConfig(options, "shudan");
        }
      });
    });
  ...
  activated: function(to, from, next) {
    ...
    if (this.shudan) {
      let desc = this.shudan.desc
        ...
        .replace(/(\n)/g, "");
      let options = {
        title: this.users.nickname + "向你推荐了一个书单，这⾥是Ta的灵魂栖息地",
        desc: this.shudan.title + "-" + desc,
        link: window.location.href,
