<template>
  <div class="orderBox">
    ...
    <div class="orderDate box_cqh">
      ...
      <router-link class="btn" :to="`/wechat/sale_order/${item.no}`" tag="div" v-if="item.type==2">订单详情</router-link>
      <router-link class="btn" :to="`/wechat/recover_order/${item.no}`" tag="div" v-if="item.type==1">订单详情</router-link>
    </div>
  </div>
</template>
