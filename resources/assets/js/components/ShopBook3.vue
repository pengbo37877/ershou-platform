      ...
      <van-card :price="skuPrice" :origin-price="skuOriginalPrice">
        ...
        <div slot="desc" class="book-info" @click="gotoBook">
          ...
          <div class="book-rating">
            豆瓣评分：{{Number(book.rating_num)==0?'暂无':book.rating_num}}
            <span
              class="level"
              v-for="item in level"
              :key="item"
        ...
        <div slot="price" v-if="this.skus.length===0">
          <span class="discount">{{getFloat((skuPrice/skuOriginalPrice)*10,1)}}折</span>
          <van-button
            round
            size="small"
        ...
        <div slot="num">
          ...
          <div
            ...
            @click="addSku"
            v-if="!inCart && !cartAdding"
          >{{skus.length>1?"多品相可选":"加入购物袋"}}</div>
  ...
  computed: {
    ...
    inCart: function() {
      ...
      var r = _this.cartCounts.find(function(item) {
        // console.log('item'+item,'book_id'+_this.book.id)
        return item == _this.book.id;
      });
      return !!r;
    },
    inReminder: function() {
