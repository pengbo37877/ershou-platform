...
//        $order = DB::transaction(function () use ($user, $userAddress, $cartItems, $order_id) {
            ...
            foreach ($cartItems as $data) {
                ...
                if(is_null($max_ship_price)){
                    $max_ship_price = $book_sku->ship_price;
                }else{
                    $max_ship_price = min($max_ship_price,$book_sku->ship_price);
                }
                $totalAmount += $book_sku->price;
            }
            // 计算运费
            if (
                $userAddress->province == '西藏自治区' ||
                $userAddress->province == '新疆维吾尔自治区' ||
                $userAddress->province == '内蒙古自治区' ||
                $userAddress->province == '海南省' ||
                $userAddress->province == '甘肃省' ||
                $userAddress->province == '青海省' ||
                $userAddress->province == '宁夏回族自治区'
            ) {
                $max_ship_price = $max_ship_price + 15;
            }
