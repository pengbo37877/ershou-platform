...
class CrawlingByWebPageISBN implements ShouldQueue
{
     ...
     * @param int $times
     */
    public function __construct($isbn, $times=1)
    {
        $this->isbn = $isbn;
        $this->times = $times;
    }
...
//        if ($book) {
//            Log::info('先不重复抓取subjectid='.$this->subjectid);
//            return;
//        }
        ...
        if (!empty($this->isbn)) {
            $html = requests::get('https://book.douban.com/isbn/' . $this->isbn .'/');
            if (!empty($html)) {
                $data = [];
                $data['isbn'] = $this->isbn;
                $all_intro = selector::select($html, "//*[@class='intro']");
                $subjectid = selector::select($html, "//meta[@http-equiv='mobile-agent']/@content");
                $subjectid = str_replace([" ", "format=html5;", "url="], '', $subjectid);
                $subjectid = Tools::findNum($subjectid);
                if (is_numeric($subjectid)) {
                    $data['subjectid'] = $subjectid;
                    Log::info('CrawlingByWebPageISBN 成功，subjectid='.$subjectid);
                }else{
                    Log::info('CrawlingByWebPageISBN 失败，subjectid='.$subjectid);
                    return;
                }
                ...
                if (isset($data['isbn']) && isset($data['subjectid']) && !empty($data['subjectid'])) {
                    $book = $this->createOrUpdateBook($data);
                    $this->fetchSecondHandInfo($html, $book);
                    $this->fetchRelation($html, $book);
                }

                Log::info($this->isbn.'=> info_array='.json_encode($info_array, JSON_UNESCAPED_UNICODE));
                Log::info($this->isbn.'=> data='.json_encode($data, JSON_UNESCAPED_UNICODE));


            }else{
                Log::info('CrawlingByWebPageISBN '.$this->isbn.' get nothing.');
                CrawlingByWebPageISBN::dispatch($this->isbn, $this->times++)->delay(now()->addSecond(3));
            }
        }
    }
    ...
    function createOrUpdateBook($data) {
        ...
        if ($book) {
            ...
            if (intval($book->subjectid)<10 && intval($data['subjectid'])>10) {
                $update_data['subjectid'] = $data['subjectid'];
            }
...
//            }
    ...
    function fetchSecondHandInfo($html, Book $book) {
        Log::info('fetchSecondHandInfo isbn='.$book->isbn);
        if (!empty($html)) {
            ...
            $douban_es_count = selector::select($html, "//a[@href=\"https://book.douban.com/subject/" . $book->subjectid . "/offers\"]");
            $douban_es_low_high = selector::select($html, "//a[@href=\"https://book.douban.com/subject/" . $book->subjectid . "/offers\"]//following-sibling::*");
            ...
            if (!$bookPrice) {
                ...
                DB::update('update book_prices set dzy_price=?, updated_at=? where id=?',
                    [$dzy_price, Carbon::now(), $bookPrice->id]);
            }
