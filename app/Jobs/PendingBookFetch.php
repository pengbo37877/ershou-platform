...
class PendingBookFetch implements ShouldQueue
{
    ...
    {
        $ps = PendingBook::select('isbn')->groupBy('isbn')->take(10)->get();
        $ps->each(function ($book){
            $this->fetchBookByIsbn($book->isbn);
        });
        $ps->each(function($book){
            PendingBook::where('isbn', $book->isbn)->delete();
        });
    }
    ...
    public function fetchBookByIsbn($isbn)
    {
        ...
        $body = $this->curl_string($url, $isbn);
        $this->buildBook($body, $isbn);
    }
...
//                Log::info('新加的书是《'.$book->name.'》isbn='.$book->isbn);
            ...
            if (floatval($book->rating_num)>9) {
                $book->update([
                    'can_recover' => true,
                    'discount' => 30
                ]);
                return;
            }else if (floatval($book->rating_num)>8.5) {
                $book->update([
                    'can_recover' => true,
                    'discount' => 25
                ]);
                return;
            }else if (floatval($book->rating_num)>8) {
                $book->update([
                    'can_recover' => true,
                    'discount' => 21
                ]);
